...
**EXCLUDE the following from brownfield detection (these are delivery tooling, not customer application code):**
- `.kiro/` directory (FDE Delivery Kit steering, skills, and settings)
- `state/` directory (FDE engagement state files)
- `aidlc-docs/` directory (AI-DLC documentation artifacts)
- `.fde-manifest.json` (FDE reference index)
- `README.md` generated by FDE render (contains "FDE Delivery Kit" header)
- MCP server configuration files (`.kiro/settings/mcp.json`, etc.)
- Any files or directories that are clearly part of the FDE Delivery Kit infrastructure

**Only classify as brownfield if the workspace contains customer application code outside of the directories and files listed above.**
