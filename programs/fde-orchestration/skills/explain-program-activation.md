...
name: "Explain Program Activation"
...
outputs:
  - name: artifact
    path: artifacts/explain-program-activation-{timestamp}.md
    kind: document
---
...
For each activated program, surface the rule that fired and the score it earned. Make the resolver decisions auditable. This skill is one of several in **FDE Orchestration Layer**; agents executing this skill should also surface related skills from the same program if the customer's situation calls for them.
