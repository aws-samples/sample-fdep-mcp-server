 ...
 * across the harness targets previously rendered for the engagement.
 *
 * Requirements: 3.5
 */
...
export interface ApplyStateUpdateInput {
  ...
  /** Path to the fdep-kit root (source of programs, pipelines, schemas). */
  kitRoot: string;
  ...
  pipelineName?: string;
  /**
   * Optional override for the harness targets to re-render. Defaults to the
...
const DEFAULT_TARGETS: HarnessTarget[] = ["kiro", "claude"];
 ...
 */
function readManifestTargets(engagementDir: string): HarnessTarget[] | undefined {
 ...
 * and re-render every harness target previously rendered for this
 * engagement.
 *
 * The flow is:
 ...
 * 2. Detect previously rendered harnesses via `render-manifest.json`,
