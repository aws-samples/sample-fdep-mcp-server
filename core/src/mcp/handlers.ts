...
export interface HandlerContext {
  /** Absolute path to the fde-kit workspace root. */
...
export async function handleRender(ctx: HandlerContext, input: RenderInput) {
  ...
  const manifest = generateFdeManifest(ctx.programs as LoadedProgram[], activeIds, input.targets);
...
export async function handleInitWorkspace(
  ...
  // Also copy the top-level fde-init steering if it exists in the kit's .kiro
  const kitSteering = join(ctx.workspaceRoot, ".kiro", "steering", "fde-init.md");
  if (existsSync(kitSteering)) {
    const destPath = join(steeringDir, "fde-init.md");
