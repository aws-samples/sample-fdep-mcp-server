...
export const MCP_TOOLS: readonly McpToolDeclaration[] = [
  ...
  },
  {
    name: "fde_render",
    description:
      "Render the resolved graph into one or more harness targets under an engagement workspace. Call this after fde_resolve. Writes steering, skills, references, and state files to the engagementDir. Returns a render report.",
    inputSchema: {
      type: "object",
      ...
      properties: {
        intentionId: { type: "string" },
        engagementDir: { type: "string", description: "Absolute path to the user's workspace root where harness artifacts should be rendered (e.g. the project directory the user has open in their IDE)." },
        targets: {
          type: "array",
          minItems: 1,
          items: {
