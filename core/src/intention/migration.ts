...
const VALID_PLATFORMS: readonly PlatformTargetId[] = [
  "kiro",
  "claude",
  "copilot",
  "cursor",
...
function toPlatformsArray(value: unknown): PlatformTargetId[] {
  if (!Array.isArray(value)) return [];
  const out: PlatformTargetId[] = [];
  const seen = new Set<string>();
  for (const item of value) {
    const s = String(item);
    if (VALID_PLATFORMS.includes(s as PlatformTargetId) && !seen.has(s)) {
      out.push(s as PlatformTargetId);
      seen.add(s);
    }
  }
  return out;
