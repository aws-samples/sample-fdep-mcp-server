/**
 * Platform Renderer — types and orchestration for per-target rendering
 * via adapters. Enforces render isolation, idempotent writes, and
...
/**
 * The closed set of supported agentic platforms (14 targets). The first
 * five are file-based adapters; the next eight are additional file-based
 * adapters; `mcp` is an MCP-style adapter that writes no files and instead
 * exposes content via the MCP server tool surface.
 */
