/**
 * Platform Renderer orchestrator — `renderAll(pipeline, targets, engagementDir)`.
 *
...
export interface RenderAllInput {
  ...
  /** List of platform targets to render. */
...
/**
 * Render the given pipeline into every requested platform target's subtree
...
async function renderTarget(
  ...
  const adapter: PlatformAdapter = getAdapter(target);
